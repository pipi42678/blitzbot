...
with open("Config\消息接收范围开始限制.txt","r",encoding="utf-8") as f:
    start_msg_wait = int(f.read())
with open("Config\消息接收范围末尾限制.txt","r",encoding="utf-8") as f:
    end_msg_wait = int(f.read())
...
def on_message(msg, chat): #比较重要的信息模块，主要作用是接收消息并发送给deepseek得到回复，然后再发送给群里
    ...
    global AI_msg, start_msg_wait, end_msg_wait, msg_cnt, msg_send_wait
